...
def image_enhancement_spatial(im_path):
    """
    Main method to change the pixels spatially
    :return: image grid
    """
    
    image_slo = plt.imread(im_path)
    ...
    enhanced_cb, cdf_cb = contrast_brightness(image_slo, 2.6, 1)
    axs[1, 0].imshow(np.uint8(enhanced_cb), cmap='gray', vmin=0, vmax=255)
    ...
    axs[2, 0].imshow(np.uint8(enhanced_gamma), cmap='gray', vmin=0, vmax=255)
    axs[2, 0].set_title("Non-linear (Gamma)")
    axs[2, 1].hist(enhanced_gamma.flatten(), 256, [0, 256], color='r')
    axs[2, 1].plot(cdf_gamma)

    enhanced_log, cdf_log = log_enhancement(image_slo, 1.5)
    #enhanced_log = cv2.equalizeHist((enhanced_log))
    axs[3, 0].imshow(np.uint8(enhanced_log), cmap='gray', vmin=0, vmax=255)
    axs[3, 0].set_title("Non-linear (Log)")
    axs[3, 1].hist(enhanced_log.flatten(), 256, [0, 256], color='r')
    axs[3, 1].plot(cdf_log)

    enhanced_gauss, cdf_gauss = gauss_enhancement(image_slo, 0.1)
    axs[4, 0].imshow(np.uint8(enhanced_gauss), cmap='gray', vmin=0, vmax=255)
    ...
    for i in range(5):
        for j in range(2):
            ...
            axs[i, j].set_yticks([])
    plt.tight_layout()
    plt.savefig(save_path + 'enhened_spatial.jpg')
    plt.show()




def image_enhancement_spectral(im_path):
    
    #Main method to change the pixels based on distribution spectrum
    #:return: image grid
    

    image_slo = cv2.imread(im_path,cv2.IMREAD_GRAYSCALE)
    ...
    enhanced_clahe, cdf_clahe = clahe_enhancement(image_slo, 15)
    ...
    for i in range(2):
        for j in range(3):
            ...
            axs[i, j].set_yticks([])
    plt.tight_layout()
    plt.savefig(save_path + 'enhened_spectral.jpg')
    plt.show()

im_path = './data/img.BMP'
save_path = './data/'
image_enhancement_spatial(im_path)
image_enhancement_spectral(im_path)
